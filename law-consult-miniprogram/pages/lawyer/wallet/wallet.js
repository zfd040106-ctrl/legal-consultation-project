  ...
  async loadWallet(isRefresh = false) {
    ...
    try {
      const [summaryResponse, flowsResponse] = await Promise.all([
        get(`/wallet/lawyer/${ownerId}`),
        get(`/wallet/lawyer/${ownerId}/flows`, {
          page: targetPage,
      ...
      this.setData({
        summary: {
          ...
          totalWithdrawn: this.formatMoney(summaryResponse.totalWithdrawn)
        },
  ...
  handleWithdraw() {
    wx.showModal({
      ...
      success: async (res) => {
        ...
        try {
          await post(`/wallet/lawyer/${this.data.ownerId}/withdraw`, {
